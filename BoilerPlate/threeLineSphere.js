...
{	
	...
	animate = function(colourIncrement, subColourIncrement, motionIncrement, rotationalIncrements=[0,0,0], trailSpeed)
	{
		//creation loop
		this.orbitCreationLoop();
		if(this.setUpStatus==0){return;}
		
		var localObjectCounter=0, pointPos, lpCounter, elipsePoints, tempPoints = new THREE.Vector2( 0, 0 );
		...
		for(localObjectCounter; localObjectCounter<this.objectTape.length; localObjectCounter++)
		{
			...
			{
				pointPos = this.pixelMap.getElipticalPointsRaw(this.objectTape[localObjectCounter-this.slices].position[0], this.objectTape[localObjectCounter-this.slices].position[1], this.objectTape[localObjectCounter-this.slices].dimensions[0], this.objectTape[localObjectCounter-this.slices].dimensions[1], this.objectTape[localObjectCounter].motionIncrements[0]+(this.trailSpacing*lpCounter));
				tempPoints = new THREE.Vector2( pointPos[0], pointPos[1] );
				this.objectTape[localObjectCounter].objects[lpCounter].position.set(tempPoints.x, tempPoints.y, this.objectTape[localObjectCounter-this.slices].position[2]);
				this.colourObject.getColour(this.subColourIndex%this.colourObject._bandWidth);
	...
	insertSphere = function()
	{
		...
		for(objectCounter=0; objectCounter<this.slices; objectCounter++)
		{
			...
			for(lpCounter=0; lpCounter<this.sliceAcuracy+1; lpCounter++)
			{
				elipsePoints = this.pixelMap.getElipticalPointsRaw(this.objectTape[objectCounter].position[0], this.objectTape[objectCounter].position[1], this.objectTape[objectCounter].dimensions[0], this.objectTape[objectCounter].dimensions[1], (360/this.sliceAcuracy)*lpCounter);
				tempPoints = new THREE.Vector2( elipsePoints[0], elipsePoints[1] );
				this.objectTape[objectCounter].shape.push(tempPoints);
			}
			this.objectTape[objectCounter].geometry.push( new THREE.BufferGeometry().setFromPoints(this.objectTape[objectCounter].shape[0]) );
		...
		for(objectCounter; objectCounter<this.slices*2; objectCounter++)
		{
			...
			{
				pointPos = this.pixelMap.getElipticalPointsRaw(this.objectTape[objectCounter-this.slices].position[0], this.objectTape[objectCounter-this.slices].position[1], this.objectTape[objectCounter-this.slices].dimensions[0], this.objectTape[objectCounter-this.slices].dimensions[1], this.objectTape[objectCounter].motionIncrements[0]+(this.trailSpacing*lpCounter));
				tempPoints = new THREE.Vector2( pointPos[0], pointPos[1] );
				...
				this.objectTape[objectCounter].materials[lpCounter].opacity = 1;//(lpCounter/this.objectTape[objectCounter].pollyPoints);
				...
				this.objectTape[objectCounter].objects[lpCounter].position.set(tempPoints.x, tempPoints.y, this.objectTape[objectCounter-this.slices].position[2]);
